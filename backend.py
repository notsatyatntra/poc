...
def call_llm(prompt: str, require_search: bool = True, context: str | None = None):
    ...
    client = ollama.Client(host="http://164.52.205.195:11434/")
    if not require_search:
        messages.pop(0)
        messages[0]["content"] = prompt

    response = client.chat(model="llama3.1", stream=True, messages=messages)
...
def get_vector_collection() -> tuple[chromadb.Collection, chromadb.Client]:
    ...
    Creates an embedding function usng Ollama and initializes a persistent ChromaDB client.
...
def get_query_context(prompt: str, collection: chromadb.Collection, min_results: int = 5) -> tuple[str | None, bool]:
    """Determines how to handle the query and retrieves context if needed.

    Args:
        prompt (str): The user query to evaluate.
        collection (chromadb.Collection): The vector collection to check for existing data.
        min_results (int, optional): Minimum number of results to consider sufficient. Defaults to 5.

    Returns:
        tuple[str | None, bool]: (context, needs_web_search)
            - context: Retrieved context from vector DB, or None if LLM can answer directly or no data is available.
            - needs_web_search: True if a web search is required, False otherwise.
    """
    prompt = prompt.lower().strip()
    
    # Check if it's a general knowledge query
    general_keywords = ["how many", "what is", "who is", "where is", "when did"]
    specific_indicators = ["latest", "recent", "current", "today", "2025", "this year", "confidential", "specific"]
    is_short = len(prompt.split()) <= 10
    starts_general = any(prompt.startswith(kw) for kw in general_keywords)
    has_specific = any(ind in prompt for ind in specific_indicators)

    #Can also use zero shot classification instead
    # how existing systems incorporate web search

    if is_short and starts_general and not has_specific:
        return None, False 
    
    # Query vector database once
    qresults = collection.query(query_texts=[prompt], n_results=10)
    documents = qresults.get("documents", [[]])[0]
    
    # Decide based on vector DB results
    if documents and len(documents) >= min_results:
        return "\n\n".join(documents), False  # Use existing vector DB context, no web search
    else:
        return None, True  # Insufficient data, web search needed
    

async def process_query(prompt: str) -> tuple[str | None, bool, chromadb.Collection, chromadb.Client]:
    collection, chroma_client = get_vector_collection()
    context, needs_web_search = get_query_context(prompt, collection)
    
    if needs_web_search:
        web_urls = get_web_urls(search_term=prompt)
        if not web_urls:
            return "No results found.", False, collection, chroma_client
        
        results = await crawl_webpages(urls=web_urls, prompt=prompt)
        add_to_vector_database(results)
        
        context = "\n\n".join([result.markdown.fit_markdown for result in results if result.markdown])
        if not context:
            return "Failed to retrieve sufficient context after crawling.", False, collection, chroma_client
    
    return context, needs_web_search, collection, chroma_client


def reset_database(chroma_client: chromadb.Client):
